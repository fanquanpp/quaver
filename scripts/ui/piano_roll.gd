...
##
## 鼠标交互（零学习成本优先）：
##   左键空白 = 新建音符并横向拖拽定长度 · 左键拖已有音符 = 移动（选区整体批量移动）·
##   抓右缘 = 改长度 · 右键 = 删除（按住扫删）· 标尺点击 = 跳播 ·
##   左侧琴键列 = 点击试听 · 底部力度条 = 拖拽改力度（选区联动）
##   Shift+拖空白 = 框选 · Shift+点音符 = 加/减选 · Ctrl+A 全选 · Delete 删除选区
## 滚轮 = 音高方向 · Shift+滚轮 = 时间方向 · Ctrl+滚轮 = 缩放 · Ctrl+0 = 重置
