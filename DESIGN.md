...
| 所有人 | "打开别是空的" | 内置示范曲《虫儿飞》双轨工程（陈光荣曲 C 调简化改编），首次启动自动加载 |
...
**【2026-09-12 最终定论】泛红真凶 = Godot 编辑器「调试 → 调试 CanvasItem 重绘」选项**（用户自查发现）。勾选后每个 CanvasItem 调用 `queue_redraw()` 时，Godot 会在其位置叠加红色调试高亮——演奏页在播放/点击时每帧重绘，视觉上就是"持续红色覆盖"；点击触发重绘，就是"点击特效"。两个关键佐证：① 该选项只影响**从编辑器启动**的实例（调试选项随运行传入），独立启动/发布版不受影响——完美解释为什么独立复现始终干净；② 红色区域形状 = 频繁重绘的控件（回声条/键盘/卷帘）轮廓，与重绘行为一一对应。**解决：取消勾选「调试 → 调试 CanvasItem 重绘」**（同菜单的显示碰撞/路径/导航/避障对本作无影响，可一并关闭）。前文的"外部合成层"推断方法论无误（该次截图的观测与排除过程仍有效），但两次现象的真正元凶均为该调试选项；白色猫形贴纸实为自动化工具自身的光标标记，与游戏无关。
...
## 24. v1.0.2 修复分析页漏挂（2026-09-12）

v1.0.0 引入混音台时误删 `tabs.add_child(ana)`，导致「分析」标签页自 v1.0.0 起从界面消失（AnalysisPanel 未入树、`_ready` 不运行，所有 `analysis.refresh()` 打印 Nil 报错）；smoke 套件因 refresh 报错不进失败清单而未被拦截。修复：恢复挂载；smoke_v014 新增断言（4 标签页、分析页/混音台在树内且已就绪）防止此类"漏挂"回归。v1.0.1 安装包刚发布即发现含此问题（无下载量），撤下并直接以 v1.0.2 替代发布。

## 25. v1.0.3 示范曲《虫儿飞》完整改编 + 演奏页音色切换 + 工具栏重排 + 高精度图标（2026-09-12）

**用户反馈**：示范曲换《虫儿飞》且"谱面完成度太差"；演奏页"没看见乐器音色切换"；按钮布局需优化；图标像素精度需提高。参照调研（GarageBand Sound Library / Ableton 浏览器 / Logic 轨道头的通行做法）：音色选择要**可见、即时试听**；走带控制固定顶栏；功能组小而可扫读。

**示范曲（完整改编，352 音符 / 32 小节）**：初版 12 小节简化谱被用户判"完成度差"。联网取得 EveryonePiano 全谱（1=F、4/4、♩=96 双手简谱，图源 `everyonepiano.cn/pianomusic/001/0000182/`），逐句转录后转 C 调。关键修正：旋律实际在**高音区**（3̇=E5），"虫儿飞"是 6→3̇→2̇ **上行跳进**而非下行；曲式 = 前奏4 + 主歌8（黑黑的天空低垂…）+ 连接1 + 主歌8（天上的星星流泪/冷风吹，主歌重复结构，仅换歌词）+ 副歌11（虫儿飞花儿睡→不管东南西北）= 32 小节；伴奏从柱式和弦升级为**分解和弦琶音**（低音-五音-根-三度 ×2/小节，源自谱面音型），半小节和声切换处（F|G 等）按谱拆分；默认音色 旋律=八音盒 / 伴奏=钢琴，BPM 96 与原谱 ♩=96 一致。G 三和弦叠旋律高音 E 时检出 Em7、半小节双和弦混检出 Dm/Gsus4 均属正常和声现象，smoke_v030 断言按 32 小节实测重写。老用户 autosave 优先于示范曲加载，需点「示范曲」听新曲。

**音色切换前置**：音色切换原本只在编曲页轨道列表的 52px 窄下拉里，用户找不到。修复分三处：① 演奏页键盘行新增「音色」组 = 轨道下拉 + 音色下拉（切换即弹 E4 试听，GarageBand 式即时反馈；鼓机轨禁用音色下拉显示"鼓组"）；② 轨道列表音色下拉加宽至 64px 并 `SIZE_EXPAND_FILL` 吃掉行内剩余宽度；③ 演奏页与轨道列表双向同步（`_sync_play_inst_ui` 挂入 `_refresh_track_ui`/`_on_track_selected`，`_syncing_play_ui` 守卫防信号回环，与琴键同步同一套路）。

**工具栏重排**：播放/停止/录制/小节位置从选项流提入顶栏（DAW 惯例：走带全局可见，任何标签页都能掌握播放状态）；选项流"走带"组改为「循环 · 音量」；原 9 项"编辑"组拆为「编辑」（撤销/重做/吸附/时值）+「缩放 · 跟随」（±/百分比/跟随/居中）——功能组越小越可扫读。

**图标重绘（40×40 高精度）**：原 6 个 32×32 图标为占位级（三角形歪、无统一描边、线性过滤缩放到 20px 发虚）。重绘方案：**20px 设计网格 ×2 放大 = 40×40 资产**，显示仍为 20px（`icon_max_width` 不变），整数 2:1 + `TEXTURE_FILTER_NEAREST` 逐像素锐利；统一 2px 描边（#14161a）+ 双色调填充（上 #d0d4dc / 下 #9aa3b2，y=10 分界）；播放三角形重心右移修正、停止圆角公式重写、导出箭头舒展；软盘/文件夹两个闲置图标接入「保存」「打开」按钮。生成管线：PIL 几何计算 → PNG → 导入 aseprite/icons.aseprite 替换旧图层（源文件保持同步）。

**自适应**：轨道列表行区包进 `ScrollContainer`（竖向滚动、横向禁用=随容器宽），轨道多/窗口矮不再裁切；演奏页提示文案补充音色组说明。

**测试**：五个 smoke 套件全过。断言同步：v014（首音 76、音符 352、MusicXML 改查 2 声部×32 小节=64 个 measure——伴奏改琶音后无同时发声组，旧 `<chord/>`=24 断言失效）；v020（撤销音色 钢琴→八音盒）；v030（32 小节断言按实测重写、段落覆盖 32）。

**【发布当日追加】修复轨道列表行空壳（v0.2.0 起的隐性缺陷）**：重拍 README 截图时发现编曲页轨道列表整列空白。排查（无头实例化 + 控件树探针）定位为 `_build_row` 构建了行内 HBox 并填满控件，但**从未执行 `row.add_child(h)`**——`git log -S` 证实该调用在 v0.2.0（d8a990c）创建 TrackList 时就不存在，行自始至终只是 6px 高的样式盒空壳（此前极矮、贴在面板顶部，未被察觉；本次加 ScrollContainer 后彻底不可见才暴露）。修复：补上挂载 + 行高显式定值 34px（ScrollContainer 内最小高度传播不可靠，交给显式约束）。附带清理 `2 / parts.size()` 整除告警（改为条件分支）。修复已并入 v1.0.3（发布仅数分钟、零下载，按 v1.0.1→v1.0.2 先例同标签替换安装包）。
