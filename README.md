...
- **音色即点即换**：「音色」组直接切换轨道与音色，切换即弹一个音试听（编曲页轨道列表同步联动）
...
- **6 种内置旋律音色 + 鼓组**，全部纯合成、力度分层（pp/mf/ff 三档）：钢琴（谐波加法+锤击瞬态）/ 芯片（25% 占空比方波）/ 柔弦（失谐正弦对）/ 贝斯 / 电钢 / 八音盒；鼓组 6 声部独立合成（底鼓扫频、军鼓噪声+鼓皮、镲不谐和方波叠…）
...
3. 按 F5 运行 —— 首次启动自带《虫儿飞》完整示范曲（32 小节：八音盒旋律 + 钢琴分解和弦，前奏-主歌×2-副歌全曲式）
...
- **测试**：`tests/smoke_v*.tscn` 五套无头回归（模型 / 走带 / 音频 / 分析 / 插件 / UI 接线），`godot --headless --path . res://tests/smoke_v100.tscn` 即跑
